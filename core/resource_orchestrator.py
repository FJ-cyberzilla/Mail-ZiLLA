...
class AdaptiveResourceOrchestrator:
        ...
        self.performance_history.append(
            {
                ...
                "resources": resources,
                "strategy_applied": (
                    self.current_strategy.level if self.current_strategy else None
                ),
            }
        )
    ...
    async def _get_network_speed(self) -> float:
        ...
        if (
            ...
            and self.last_network_test
            and datetime.now() - self.last_network_test < timedelta(minutes=5)
        ):
    ...
    def determine_resource_strategy(
        ...
        memory_score = max(
            0, 100 - resources.memory_usage
        )  # Higher available memory = better
        ...
        self.logger.info(
            f"🎯 Resource strategy: {level.value} (score: {overall_score:.1f})"
        )

        return strategy

    def _create_strategy_for_level(
        self, level: ResourceLevel, resources: SystemResources
    ) -> ResourceStrategy:
    ...
    async def optimize_agent_operations(
        self, strategy: ResourceStrategy
    ) -> Dict[str, Any]:
        ...
        optimizations = {
            ...
            "data_quality": {
                "enable_image_analysis": strategy.data_quality
                in ["comprehensive", "standard"],
                ...
                "max_profiles_per_platform": (
                    50 if strategy.data_quality == "comprehensive" else 20
                ),
            },
            "caching": {
                ...
                "cache_ttl_minutes": (
                    60 if strategy.caching_strategy == "aggressive" else 30
                ),
            },
            "proxy_management": {
                "proxy_rotation_frequency": (
                    "high" if strategy.proxy_usage == "aggressive" else "medium"
                ),
                "max_proxy_retries": 3 if strategy.proxy_usage == "aggressive" else 2,
                "proxy_health_check_interval": (
                    300 if strategy.proxy_usage == "aggressive" else 600
                ),
            },
        }
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        avg_memory_usage = sum(
            p["resources"].memory_usage for p in recent_performance
        ) / len(recent_performance)
        avg_cpu_usage = sum(p["resources"].cpu_usage for p in recent_performance) / len(
            recent_performance
        )
        avg_network_speed = sum(
            p["resources"].network_speed for p in recent_performance
        ) / len(recent_performance)

        current_strategy = (
            self.current_strategy.level.value if self.current_strategy else "unknown"
        )
    ...
    def _analyze_resource_trend(self) -> str:
        ...
        avg_recent_memory = sum(p["resources"].memory_usage for p in recent) / len(
            recent
        )
