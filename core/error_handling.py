...
class ErrorHandler:
    ...
    async def handle_agent_error(self, agent_name: str, error: Exception, context: Dict = None):
        """Handle agent-specific errors with recovery strategies"""
        error_key = f"agent_{agent_name}_{type(error).__name__}"
        self.error_counts[error_key] = self.error_counts.get(error_key, 0) + 1
        
        # Log error with context
        self.logger.error(f"Agent {agent_name} error: {error}", extra={
            'context': context,
    ...
    async def _trigger_alert(self, agent_name: str, error: Exception):
        ...
        self.logger.critical(f"CRITICAL: Agent {agent_name} has recurring errors: {error}")
