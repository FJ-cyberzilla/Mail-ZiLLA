...
def setup_logger(name: str = "cyberzilla", log_level: str = "INFO") -> logging.Logger:
    ...
    file_handler = RotatingFileHandler(
        "logs/cyberzilla.json.log", maxBytes=10 * 1024 * 1024, backupCount=5  # 10MB
    )
    file_handler.setFormatter(JSONFormatter())

    # Error file handler
    error_handler = RotatingFileHandler(
        "logs/cyberzilla.errors.log", maxBytes=5 * 1024 * 1024, backupCount=3  # 5MB
    )
...
def get_security_logger():
    ...
    audit_handler = RotatingFileHandler(
        "logs/security_audit.log", maxBytes=5 * 1024 * 1024, backupCount=10
    )
