...
@app.task(
    bind=True, name="tasks.social_lookup_task", max_retries=3, default_retry_delay=60
)
def social_lookup_task(
    self, email: str, advanced_analysis: bool = False, user_context: Dict = None
):
    """
    ...
    try:
        ...
        if advanced_analysis:
            ...
            result = asyncio.run(social_agent.process_email(email, user_context))
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.info(
                f"🔄 Retrying task {task_id} in {retry_delay}s (attempt {self.request.retries + 1})"
            )
    ...
    for email in emails:
        try:
            ...
            result = social_lookup_task.apply_async(
                args=[email, False, user_context], queue="social_lookup"
            )
            results.append(
                {"email": email, "task_id": result.id, "status": "submitted"}
            )
    ...
    try:
        ...
        result = asyncio.run(
            social_agent.process_email_enterprise(
                email, user_context, collect_fingerprint=True
            )
        )
