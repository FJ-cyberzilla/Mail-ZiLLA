...
class GitHubAgent(BaseCodeAgent):
    ...
    async def search_by_email(self, email: str, context: Dict[str, Any] = None) -> List[ProfileData]:
        ...
        for strategy in strategies:
            try:
                ...
                if strategy_profiles:
                    self.logger.info(f"✅ Found {len(strategy_profiles)} GitHub profiles")
                    break
        ...
        self.logger.info(f"📊 GitHub search completed: {len(enriched_profiles)} profiles found")
    ...
    async def _search_github_api(self, email: str, context: Dict[str, Any]) -> List[ProfileData]:
        ...
        try:
            ...
            success, response = await self._make_request('GET', search_url, params=params, headers=headers)
            if success:
                data = json.loads(response)
    ...
    async def _search_github_commits(self, email: str, context: Dict[str, Any]) -> List[ProfileData]:
    ...
    async def _search_github_organizations(self, email: str, context: Dict[str, Any]) -> List[ProfileData]:
    ...
    async def _get_user_profile(self, username: str) -> Optional[ProfileData]:
        ...
        try:
            ...
            success, response = await self._make_request('GET', user_url, headers=headers)
            if not success:
                return None
    ...
    async def get_repositories(self, username: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            success, response = await self._make_request('GET', repos_url, params=params, headers=headers)
            if success:
    ...
    async def analyze_activity(self, username: str) -> Dict[str, Any]:
        ...
        try:
            ...
            success, response = await self._make_request('GET', events_url, headers=headers)
            if success:
                ...
                return {
                    ...
                    'primary_activity': max(event_counts, key=event_counts.get) if event_counts else None
                }
