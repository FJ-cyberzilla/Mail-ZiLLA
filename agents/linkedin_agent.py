...
class LinkedInAgent(BaseSocialAgent):
    ...
    async def search_by_email(
        ...
        for strategy in strategies:
            try:
                ...
                high_confidence_profiles = [
                    p for p in strategy_profiles if p.confidence > 0.7
                ]
                if high_confidence_profiles:
                    self.logger.info(
                        f"✅ Found {len(high_confidence_profiles)} high-confidence LinkedIn profiles"
                    )
                    break
        ...
        self.logger.info(
            f"📊 LinkedIn search completed: {len(unique_profiles)} unique profiles found"
        )
    ...
    async def _search_linkedin_direct(
        ...
        try:
            ...
            search_url = (
                f"{self.platform_config['base_url']}/sales-api/salesApiProfiles"
            )
            payload = {
                "query": {"emailAddress": email, "searchScope": "ALL"},
                "start": 0,
                "count": 25,
            }
    ...
    async def _search_linkedin_advanced(
        self, email: str, context: Dict[str, Any]
    ) -> List[ProfileData]:
        ...
        try:
            ...
            # Advanced search URL
            search_url = (
                f"{self.platform_config['base_url']}/voyager/api/search/cluster"
            )
            params = {
    ...
    async def _search_linkedin_sales_navigator(
        self, email: str, context: Dict[str, Any]
    ) -> List[ProfileData]:
        ...
        try:
            ...
            nav_url = (
                f"{self.platform_config['base_url']}/sales-api/salesApiEngagements"
            )
            payload = {"filters": {"emailAddress": email}, "start": 0, "count": 10}
    ...
    def _parse_linkedin_api_response(
        self, response_data: str, original_email: str
    ) -> List[ProfileData]:
    ...
    def _parse_linkedin_advanced_results(
        self, response_data: str, original_email: str
    ) -> List[ProfileData]:
        ...
        try:
            ...
            for cluster in data.get("elements", []):
                for item in cluster.get("items", []):
                    profile = self._extract_profile_from_search_item(
                        item, original_email
                    )
                    if profile:
                        profiles.append(profile)
    ...
    def _parse_sales_navigator_response(
        self, response_data: str, original_email: str
    ) -> List[ProfileData]:
        ...
        try:
            ...
            for profile_data in data.get("elements", []):
                profile = self._extract_profile_from_navigator(
                    profile_data, original_email
                )
                if profile:
                    profiles.append(profile)
    ...
    async def _extract_profile_from_element(
        self, element: Dict, original_email: str
        ...
        try:
            ...
            profile_url = (
                f"https://www.linkedin.com/in/{profile_info.get('vanityName', profile_id)}"
                if profile_id
                else None
            )
            ...
            profile = ProfileData(
                ...
                confidence=self._calculate_linkedin_confidence(
                    profile_info, original_email
                ),
    ...
    def _extract_profile_from_search_item(
        self, item: Dict, original_email: str
    ) -> Optional[ProfileData]:
    ...
    def _extract_profile_from_navigator(
        self, profile_data: Dict, original_email: str
    ) -> Optional[ProfileData]:
    ...
    def _extract_company_info(self, profile_info: Dict) -> Dict[str, str]:
        ...
        try:
            ...
            current_position = next(
                (p for p in positions if p.get("isCurrent", False)), None
            )

            if current_position:
                company_info["company"] = current_position.get("company", {}).get(
                    "name"
                )
    ...
    def _calculate_linkedin_confidence(
        self, profile_info: Dict, original_email: str
    ) -> float:
    ...
    def _parse_detailed_profile(
        self, profile_data: Dict, profile_url: str
    ) -> ProfileData:
    ...
    def _extract_current_company(self, profile_info: Dict) -> Optional[str]:
        ...
        try:
            ...
            return (
                current_position.get("company", {}).get("name")
                if current_position
                else None
            )
        except Exception:
