...
class TwitterAgent(BaseSocialAgent):
    ...
    async def search_by_email(self, email: str, context: Dict[str, Any] = None) -> List[ProfileData]:
        ...
        try:
            ...
            success, response = await self._make_request('GET', users_url, params=params, headers=headers)
            if success:
    ...
    async def _search_by_name(self, name: str, original_email: str) -> List[ProfileData]:
    ...
    def _parse_twitter_users(self, response_data: Dict, identifier: str) -> List[ProfileData]:
    ...
    def _parse_twitter_user(self, user_data: Dict, identifier: str) -> Optional[ProfileData]:
        ...
        try:
            ...
            profile = ProfileData(
                ...
                company=self._extract_company_from_bio(user_data.get('description', '')),
    ...
    async def search_by_username(self, username: str, context: Dict[str, Any] = None) -> List[ProfileData]:
        ...
        """
        try:
            users_url = f"{self.platform_config['base_url']}/2/users/by/username/{username}"
            params = {
                'user.fields': 'created_at,description,location,name,profile_image_url,public_metrics,url,username,verified'
            }
