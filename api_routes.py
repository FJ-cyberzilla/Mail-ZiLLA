...
from tasks.worker_tasks import social_lookup_task, batch_lookup_task, urgent_analysis
...
async def social_lookup(
    ...
    await rate_limiter.check_rate_limit(
        identifier=f"lookup_{user_id}",
        limit=2,  # 2 lookups per hour
        window=3600
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to process lookup request"
        )
...
async def batch_lookup(
    ...
    if len(request.targets) > 10:
        raise HTTPException(
            status_code=422,
            detail="Maximum 10 targets allowed per batch request"
        )
    ...
    for target in request.targets:
        ...
        if '@' in sanitized_target:
            ...
            if len(sanitized_target) < 10:
                raise HTTPException(
                    status_code=422,
                    detail=f"Invalid phone format: {sanitized_target}"
                )
        else:
            raise HTTPException(
                status_code=422,
                detail=f"Invalid target format: {sanitized_target}"
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to process batch lookup request"
        )
    ...
    try:
        with db_manager.get_db() as db:
            ...
            return LookupResponse(
                ...
                platforms_found=list(set(p['platform'] for p in (task.profiles_found or []))),
...
async def urgent_analysis_request(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to process urgent analysis request"
        )
    ...
    try:
        ...
        for agent_id, details in performance_report.get('agent_details', {}).items():
            agents_status.append(AgentStatusResponse(
                ...
                is_healthy=details.get('performance_rating') in ['excellent', 'good']
            ))
...
async def get_system_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve system performance"
        )
