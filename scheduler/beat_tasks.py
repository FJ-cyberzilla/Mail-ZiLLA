...
from database.models import ProxyPool, SystemMetrics, TaskResult
...
def proxy_health_check():
    ...
    try:
        ...
        with db_manager.get_db() as db:
            ...
            active_proxies = (
                db.query(ProxyPool).filter(ProxyPool.is_active == True).all()
            )
            ...
            for proxy in active_proxies:
                try:
                    ...
                    success_rate = (
                        total_requests - proxy.failed_requests
                    ) / total_requests
                    ...
                    logger.error(
                        f"Proxy health check failed for {proxy.proxy_url}: {e}"
                    )
        ...
        logger.info(
            f"✅ Proxy health check completed: {checked_count} checked, {healthy_count} healthy, {deactivated_count} deactivated"
        )
...
def refresh_proxy_pool():
    ...
    try:
        ...
        try:
            new_proxies = loop.run_until_complete(proxy_manager.auto_acquire_proxies())
            ...
            with db_manager.get_db() as db:
                ...
                for proxy_url in new_proxies:
                    existing = (
                        db.query(ProxyPool)
                        .filter(ProxyPool.proxy_url == proxy_url)
                        .first()
                    )
            ...
            logger.info(
                f"✅ Proxy pool refreshed: {len(new_proxies)} new proxies added"
            )
...
def system_health_check():
    ...
    try:
        ...
        logger.info(
            f"✅ System health check completed: {health_data['overall_status']}"
        )
...
def calibrate_ai_agents():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"✅ AI agents calibrated: {performance_report['overview']['overall_success_rate']:.1%} success rate"
            )
...
def _gather_training_data() -> List[Dict]:
    ...
    with db_manager.get_db() as db:
        ...
        recent_analyses = (
            db.query(TaskResult)
            .filter(TaskResult.status == "completed", TaskResult.confidence_score > 0.8)
            ...
            training_data.append(
                {
                    ...
                    "complexity": len(analysis.profiles_found or [])
                    / 10.0,  # Normalized
...
def cleanup_old_task_results():
    ...
    try:
        ...
        logger.info(
            f"✅ Cleanup completed: {deleted_count} tasks, {deleted_metrics} metrics removed"
        )
...
def generate_nightly_report():
    ...
    try:
        ...
        with db_manager.get_db() as db:
            ...
            tasks_today = (
                db.query(TaskResult).filter(TaskResult.created_at >= today).count()
            )

            successful_tasks = (
                db.query(TaskResult)
                .filter(
                    TaskResult.created_at >= today, TaskResult.status == "completed"
                )
                .count()
            )

            avg_confidence = (
                db.query(func.avg(TaskResult.confidence_score))
                .filter(
                    TaskResult.created_at >= today, TaskResult.status == "completed"
                )
                .scalar()
        ...
        logger.info(
            f"✅ Nightly report generated: {tasks_today} tasks, {successful_tasks} successful"
        )
