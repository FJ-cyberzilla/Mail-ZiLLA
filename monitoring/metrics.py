...
# Metrics definitions
SEARCH_REQUESTS = Counter('search_requests_total', 'Total search requests', ['platform', 'status'])
...
def record_search_metrics(platform: str, duration: float, success: bool):
    SEARCH_REQUESTS.labels(platform=platform, status='success' if success else 'error').inc()
